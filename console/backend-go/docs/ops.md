...
# 按 Ctrl+C 停止所有服务；或在另一终端执行 ./console/stop-go.sh
```
...
| 脚本 | 作用 |
|---|---|
| `./console/start-go.sh` | 启动 agent + Go 后端（自动补依赖、构建前端、预编译二进制），Ctrl+C 停止 |
| `./console/start-go-mtls.sh` | 以 **mTLS 模式**启动 agent + Go 后端（证书缺失时自动生成） |
| `./console/stop-go.sh` | 读取 `console/.pids/` 中的 PID 安全停止（含 mTLS 进程） |
...
一键脚本会把进程 PID 写入 `console/.pids/`（如 `agent-go.pid`、`console-go.pid`），`stop-go.sh` 据此精确停止，避免误杀其他进程。
...
| 端口被占用 `http server failed` | 8081 已被其他进程使用 | 改 `PRIVACY_CONSOLE_PORT`，或 `./console/stop-go.sh` 清理残留 |
