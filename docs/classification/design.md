...
本章用于将 `privacy_local_agent/privacy/classification*` 相关实现统一纳入项目审核标准，重点衡量：

1. **可观测**：是否能通过日志、指标和审计信息定位问题。
2. **可验证**：是否存在清晰的输入校验、边界处理与失败返回。
3. **可维护**：是否遵循统一的类型、抽象与代码风格。
4. **可交付**：是否具备可供评审、联调和回归验证的文档证据。

### 13.2 评分口径

每个维度按 0~5 分评分，建议采用以下口径：

| 分值 | 判定标准 |
|---|---|
| 0 | 缺失该能力，或存在明显反向实现（例如只打印原始数据、无校验、无类型约束）。 |
| 1 | 有零散实现，但无法支撑生产使用；覆盖面很低。 |
| 2 | 具备基础实现，但关键路径仍有缺口，且缺乏一致性。 |
| 3 | 达到可用水平，主流程具备该能力，但边界场景或证据不足。 |
| 4 | 实现完整，覆盖主要路径，具备较好的可维护性与可追溯性。 |
| 5 | 实现完整且标准化，关键路径、边界场景、文档证据均齐备，可直接作为项目审核样板。 |
...
- **文档证据**：是否具备中英文注释或 docstring，是否包含 Execution Steps 或等价流程说明。
...
- 只有单一 Counter：通常不高于 3 分。
- 只有局部埋点、无法覆盖主路径：1~2 分。
- 指标标签维度过多或含敏感信息：0~1 分。

#### 13.4.3 双语文档

满分 5 分需满足：

- 核心类、核心函数具备清晰 docstring。
- 关键设计说明同时包含中文说明与英文关键术语。
- 至少对执行步骤、输入输出、边界条件、失败模式做出说明。
- 文档与实现保持同步，避免“文档正确、代码不一致”。
...
- 对所有公共入口参数进行校验，包括类型、取值范围、空值、枚举值、长度与依赖关系。
- 对非法输入应快速失败，并返回明确错误信息。
- 对外部依赖不可用、配置缺失、模型缺失等情况有明确降级或异常路径。
- 对批量接口应支持部分失败可定位，避免吞错。

扣分建议：

- 只有部分字段校验：2~3 分。
- 依赖后续逻辑自然报错：1~2 分。
- 输入非法时返回模糊异常或 500：0~1 分。
...
- 使用明确的 type hints。
- 优先使用 `Enum`、`dataclass`、Pydantic model 等表达结构化领域对象。
- 函数职责单一，命名清晰，避免重复实现与魔法常量。
- 公共 API 保持幂等、可推断、可测试。
...
| 总分 | 结论 | 含义 |
|---|---|---|
| 24~25 | 通过，且可作为标杆 | 可直接复用为团队最佳实践。 |
| 20~23 | 通过 | 满足工业化要求，可进入主线。 |
| 16~19 | 有条件通过 | 需补齐部分证据或修复缺口后复审。 |
| 0~15 | 不通过 | 存在明显工程化短板，不建议进入主线。 |
