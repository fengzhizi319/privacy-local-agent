...
Go 后端位于 `frontend/backend-go/`，是测试控制台与 Python Agent 之间的**网关代理**：把前端的 REST/JSON 请求转换为 gRPC 调用转发给 Agent，同时可直接托管前端静态资源。
...
- 依赖见 [go.mod](../frontend/backend-go/go.mod)：`go 1.25.0` + `gin v1.12.0` + `grpc v1.82.1`；
- 入口 [cmd/server/main.go](../frontend/backend-go/cmd/server/main.go)：加载配置 → 建 gRPC 连接 → 注册路由 → 启动 HTTP 服务 → **监听 SIGINT/SIGTERM 优雅关闭**（5 秒超时，等待活跃请求完成）；
...
前端位于 `frontend/web/`，是用于验证 Agent 全部能力的测试控制台。
...
[vite.config.ts](../frontend/web/vite.config.ts) 中配置：

- `@` 路径别名指向 `src/`，简化导入；
- 开发代理 `/api → http://127.0.0.1:8080`（`changeOrigin: true`），作为同源回退方案；
- 构建产物输出到 `dist/`，由 Python/Go 后端同源托管（详见 [backend/docs/ops.md](../frontend/backend/docs/ops.md) 的开发/生产模式与 CORS 方案）。
...
- API 调用集中在 [src/api/client.ts](../frontend/web/src/api/client.ts)，通过可变基址 `API_BASE` 支持双后端切换（`BackendSelector`）；
- 数据文件解析（[src/utils/fileParse.ts](../frontend/web/src/utils/fileParse.ts)）用状态机实现 CSV 解析，与后端 records 语义对齐，支撑"处理前后对比"视图。
