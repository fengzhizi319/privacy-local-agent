...
本文档描述控制台 Python 后端（`frontend/backend`）对外提供的全部 REST 接口、请求/响应数据模型与环境变量配置。
...
控制台另有 Go gRPC 代理后端（`frontend/backend-go`，默认 `8081`），接口契约（`/api/health` / `/api/samples` / `/api/proxy` / `/api/batch`）与本文档保持一致，差异在于：
